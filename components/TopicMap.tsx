...
// ─── Map user course string to CourseCombo ──────────────────────
...
// ─── Fibonacci sphere for natural sub-node distribution ──────────
function fibSphere(i:number, total:number, radius:number):{x:number;y:number;z:number} {
  const phi = Math.acos(1 - 2*(i+0.5)/total);
  const theta = Math.PI*(1+Math.sqrt(5))*i;
  return {
    x: radius*Math.cos(theta)*Math.sin(phi),
    y: radius*Math.sin(theta)*Math.sin(phi)*0.7, // slight vertical compression for topic-map feel
    z: radius*Math.cos(phi),
  };
}

// ─── Tree builder — ~500-700 nodes ─────────────────────────────
function buildTree(combo:CourseCombo):{nodes:N3[];edges:E3[]} {
  ...
  // Hubs — spread widely along Z axis for clear separation
  ...
  // Center the universe — shift all nodes so hub centroid is at origin
  ...
  // Topic definitions — 2024 NESA syllabus, all start dim
  ...
  // Deterministic seed per topic for stable builds
  let seed=combo.length*137;
  const rng=()=>{seed=(seed*16807+0)%2147483647;return(seed-1)/2147483646;};

  // Place depth-1 topics on a wider 3D shell — each course category well-separated
  for(let i=0;i<allTopics.length;i++){
    ...
    // ── Depth 2: 8-11 sub-nodes per topic (Fibonacci sphere) ──
    ...
    for(let j=0;j<numD2;j++){
      ...
      // ── Depth 3: 1-3 leaves per depth-2 ──
...
const TopicMap:React.FC=()=>{
  ...
  // Initialize combo from user's enrolled course
  useEffect(() => {
  ...
  // Apply user progress to tree nodes — mark which have real data
  // ALWAYS runs (even when progress is empty) to ensure untrained nodes stay dim
  useEffect(() => {
    setTree(prev => {
  ...
  useEffect(()=>{
    ...
    // ── Natural motion: acceleration, multi-axis, breathing zoom ──
    const motion = {
      t: Math.random()*1000,
      // Actual velocities (change slowly, giving acceleration feel)
      velRx: 0, velRy: 0, velRz: 0,
      // Target velocities drift over time
      targetRyBase: 0.0025,
    ...
    const loop=()=>{
      ...
      // ── Google Earth-style organic rotation: slower, random momentum ──
      ...
      // Smooth toward orbit target when focused
      ...
      for(const e of tree.edges){
        ...
        // Skip very long edges (perspective outliers)
      ...
      // ── Nodes: solid sphere core + soft surrounding glow ──
      ...
      for(const n of projected){
        ...
        const hasData = (n as any)._hasData !== false; // true if user has practiced this
        const rgb=CAT_RGB[n.cat]||[16,185,129];
        const isGhost=mastered<0.03&&n.depth>1;
        const hb=isHov||isSel?1.4:1;
        // mFactor: 0.08 for untrained, up to 1.0 for mastered
        const mFactor = hasData ? Math.max(0.15, mastered) : 0.08;

        // ── Outer glow — DRAMATIC for high mastery, dim for untrained ──
        ...
        // ── Solid sphere core — vibrant for mastered, grey for untrained ──
        const sr=scaledR*hb;
        ...
        if(n.depth===0){
          ...
          // Untrained nodes: grey, barely visible
          sg.addColorStop(0,'rgba(63,63,70,0.25)');
          sg.addColorStop(0.5,'rgba(63,63,70,0.04)');
          sg.addColorStop(1,'rgba(0,0,0,0)');
        }else if(n.depth===1){
          // Mastery drives sphere vibrancy — bright core + colored corona
        ...
        // ── Hub rings (decorative, not mastery) ──
        if(n.depth===0){
  ...
  const handleClick=(e:React.MouseEvent)=>{
    ...
    // Find clicked node (depth 0-1 only for popup)
    ...
    if(found){
      // ── Fix 1: orbit camera to face the clicked node ──
      const node=tree.nodes.find(n=>n.id===found.id);
      if(node){
        // Point camera at the node and zoom in to show surrounding adjacency clearly
        ...
        orbitT.current={
          ...
          rz: 0.05,
          zm: Math.max(200, Math.min(400, 250 + dist*0.3))  // tighter zoom to show adjacent nodes
        };
  ...
  // ── Popup anchored to viewport edge, never over the node ──
  ...
  return(
    <div ref={boxRef} className="w-full h-full min-h-screen relative bg-[#07080a] overflow-hidden">
      ...
      {/* Popup — vape.gg style with gradient glow */}
      <AnimatePresence>
        {popup&&popupStyle&&(
          <motion.div
